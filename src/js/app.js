...
const FullPage = new fullpage("#fullpage", {
  ...
  onSlideLeave: function (section, origin, destination, direction) {
    const nextContainer = document.getElementsByClassName(destination.anchor)[0].getElementsByClassName('container')[0];
    nextContainer.classList.remove('animation-right');
    nextContainer.classList.remove('animation-left'); 
    
    window.requestAnimationFrame(() => {
      direction === 'right' ? nextContainer.classList.add('animation-right') : nextContainer.classList.add('animation-left')      
    })   
